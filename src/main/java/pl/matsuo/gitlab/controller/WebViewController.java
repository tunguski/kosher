...
public class WebViewController {
  ...
  @Autowired GitRepositoryService gitRepositoryService;
  @Autowired GenerateContentService generateContentService;
        ...
        .map(
            config -> {
              ...
              if (body != null) {
                ...
                String generated =
                    generateContentService.generate(
                        user, project, branch, request, config, properties);
                ignoreEx(
                    () ->
                        writeStringToFile(
                            new File(
                                new File(config.getParentFile(), properties.destination()),
                                restOfTheUrl),
                            generated));
                return generated;
              }
  ...
  @ResponseStatus(HttpStatus.OK)
  public @ResponseBody FileSystemResource getResource(
      @PathVariable("user") String user,
      @PathVariable("project") String project,
      @PathVariable("branch") String branch,
      HttpServletRequest request) {
