 ...
 * such as {@link #orElse(Object) orElse()} (return a default value if value not present) and {@link
 * #ifPresent(Consumer) ifPresent()} (execute a block of code if the value is present).
 *
 ...
 * synchronization) on instances of {@code Optional} may have unpredictable results and should be
 * avoided.
 *
 * @since 1.8
 */
public final class Optional<T> {
   ...
   *
   * <p>Generally only one empty instance, {@link Optional#EMPTY}, should exist per VM.
   */
  private Optional() {
    this.value = null;
  }
   ...
   * {@code ==} against instances returned by {@code Option.empty()}. There is no guarantee that it
   * is a singleton. Instead, use {@link #isPresent()}.
   *
   ...
   * Optional}.
   *
   * <p>This method supports post-processing on optional values, without the need to explicitly
   * check for a return status. For example, the following code traverses a stream of file names,
   * selects one that has not yet been processed, and then opens that file, returning an {@code
  ...
  public Optional<T> or(T other) {
    return value != null ? this : new Optional<>(other);
  }
   ...
   * @throws NullPointerException if value is not present and {@code other} is null
   */
  public Optional<T> orGet(Supplier<? extends T> other) {
    return value != null ? this : new Optional<>(other.get());
  }
   ...
   * @throws NullPointerException if no value is present and {@code exceptionSupplier} is null
   */
  public <X extends Throwable> Optional<T> orThrow(Supplier<? extends X> exceptionSupplier)
      throws X {
    if (value != null) {
      return this;
    } else {
   ...
   *
   * <p>If a value is present the result must include its string representation in the result. Empty
   * and present Optionals must be unambiguously differentiable.
   *
