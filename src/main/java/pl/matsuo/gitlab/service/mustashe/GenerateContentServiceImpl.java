...
public class GenerateContentServiceImpl implements GenerateContentService {
  ...
  PegDownProcessor processor =
      new PegDownProcessor(TABLES | DEFINITIONS | FENCED_CODE_BLOCKS | AUTOLINKS, 30000);
  ...
  protected String processTemplate(
      String template, File config, JekyllProperties properties, MultiSourceValueProvider provider)
      throws IOException {
    ...
    while (process) {
      ...
      // layout processing
      String layout =
          provider.get(conf -> conf.get("layout") != null ? conf.get("layout").asText() : null);
      if (layout != null) {
        String layoutBody = readFile(config, properties, "_layouts/" + layout + ".html");
        template =
            layoutBody
                .replaceAll("\\{\\{\\s*content\\s*\\}\\}", template.replaceAll("\\$", "\\\\\\$"))
                .trim();
      }
  ...
  protected String replaceComplex(
      String regex,
      String template,
      BiConsumer<Matcher, StringBuffer> modifier,
      BiConsumer<Matcher, StringBuffer> tail) {
  ...
  private JsonNode buildModel(
      String user, String project, String branch, HttpServletRequest request) throws IOException {
    String restOfTheUrl =
        ((String) request.getAttribute(PATH_WITHIN_HANDLER_MAPPING_ATTRIBUTE))
            .replaceFirst("/" + user + "/" + project + "/" + branch, "")
            .replaceAll("/+", "/");
    ...
    return mapper.readTree(
        "project:"
            ...
            + branch
            + "\n  href: "
            + getBaseHref(user, project, branch, request)
            + "\nbase: "
            + base
            + "\nbase_gitlab: "
            + gitlabServer);
  ...
  protected String mustache(
      File config, JekyllProperties properties, String template, MultiSourceValueProvider provider)
      throws IOException {
  ...
  protected String getBaseHref(
      String user, String project, String branch, HttpServletRequest request) {
    String url = request.getRequestURL().toString();
    // todo: base href must contain absolute url - extract from request
    return url.substring(
            0,
            url.indexOf(String.join("/", user, project, branch))
